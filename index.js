...
function upload(receiver, configurePath, data, release, content, file, callback) {
  var subpath = file.subpath;
  var publishPath= file.release;
  var from=configurePath["from"];
  var regFrom = new RegExp("(^\/bin\/)?" + from + "/");
  var to=configurePath["to"];
  data['to'] = _(path.join(to, release));
  if(regFrom.test(publishPath)||typeof(from)==="undefined"){
    fis.util.upload(
...
module.exports = function(arr_options, modified, total, callback) {
  if(typeof(arr_options.__pos)==="undefined"){
...
function handleSingle(options, modified, total, callback){
  ...
  modified.forEach(function(file) {
    ...
    steps.push(function(next) {
      ...
      upload(receiver, configurePath, data, file.getHashRelease(), file.getContent(), file, function(error) {
        if (error) {
